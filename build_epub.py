...
def extract_summaries(summary_path):
    """Extracts text between 'SUMMARY:' and 'CONTENT:' from the .out.txt file."""
...
def clean_chapter_title(ch_text, global_count):
    """Filters titles and returns a sequential Arabic title."""
...
def clean_chapter_body(ch_text):
    """Removes all metadata, boundary markers, and technical headers."""
    # Remove technical headers and labels
    ...
    # Remove boundary lines and markers
...
def build_epub(input_txt, summary_txt, output_epub):
    ...
    book = epub.EpubBook()
    book.set_identifier('id_clean_bilingual_vfinal')
    book.set_title('The Wings of the Dove - Clean Edition')
    ...
    for ch_text in raw_chapters:
        ...
        if new_title:
            ...
            cleaned_text = clean_chapter_body(ch_text)
            
            # The HTML structure with the gray background summary box
            chapter_content = f'''
    ...
    # Styling for the gray background and clean text
    style = '''
        ...
        .summary-content { 
            padding: 15px; 
            background-color: #f6f6f6; /* The gray background for the revealed text */
...
if __name__ == "__main__":
    ...
    parser.add_argument("--summary_file", required=True) # Restoration of your specific flag

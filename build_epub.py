...
def clean_text_block(html_content):
    """Strips technical headers and meta-talk markers from the final EPUB."""
    cleaned = re.sub(r'### SECTION \d+ ORIGINAL', '', html_content)
    cleaned = cleaned.replace("Enough thinking", "") # Catch-all for AI artifact
    cleaned = re.sub(r'={6,}', '', cleaned)
    return cleaned.strip()

def create_bilingual_epub(txt_source, output_epub):
    book = epub.EpubBook()
    book.set_identifier('id_wings_dove_v2_final_restored')
    ...
    # THE TOC FIX: Regex that catches <h2 class="xhtml_h2_align" id="pgepubid00009"...>
    ...
    for idx, i in enumerate(range(1, len(chapter_chunks), 2)):
        ...
        # Build the chapter HTML using the original cleaned body
    ...
    print(f"\n[SUCCESS] Production EPUB built with TOC ({len(toc_links)} chapters).")
